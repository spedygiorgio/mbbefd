...
The corresponding log-normal parameters $\mu$ $\sigma$ can be derived via:
...
$f(x)$, the cumulative distribution function $F(x)$ and the survival function $S(x)=1-F(x)$. 
...
In the insurance context the label *exceedance probability function* is often used for the survival function, as it describes the 
probability of exceeding a certain loss.

# Expected Loss

Let's define $X$ as the random variable that describes the loss.
The expected loss (in plain English the probability weighted sum of the losses) is then:

$$
E[X] = \int_{-\infty}^\infty x\, f(x) dx
$$

Assuming losses are not negative ($P(X<0)=0$) 
this simplifies to:
...
If I further assume that losses are limited by $\alpha$ in my contract. Then
...
ILFs are often used for pricing casualty business.
...
But we will require a view on the expected full value loss cost for the overall
exposure. 

Further we assume that the damage ratio is independent of the size 
of the risk.
...
or perhaps as the maximum probable loss (MPL). Other definitions are
also popular, such as possible maximum loss (PML), estimate maximum loss (EML),
or maximum foreseeable loss (MFL).

Defining the 'right' exposure metric can be tricky.

While the TIV is straightforward to understand, the MPL is a little more
challenging. Suppose we insure a big production plant, consisting of 
several buildings, against fire. It is unlikely that a fire will destroy 
the whole facility, instead it is believed that the distance between the 
building will ensure that fires can be contained in a local area. Hence 
the MPL might be only the highest value of any of those buildings. Instead using the loss amount as metric we use the degree of loss 
as a % of the exposure metric.
...
Thus, the insurer needs to understand, by how much the claims burden is reduced 
...
The exposure curve (or also called First Loss Curve) is defined as 
the proportion of the LEV for a given deductible $d$ compared to 
...
```{r ExposureCurve, dev.args=list(pointsize=8), fig.width=3.5, fig.width=5}
MPL <- 200
ExpectedLoss <- 65
...
one parameter $c$, as shown by Bernegger.

```{r mbbefd}
mbbefdCurve <- function(x, C){
  g <- exp((0.78 + 0.12*C)*C)
  b <- exp(3.1 - 0.15*(1+C)*C) 
...
```{r mbbefdPlot, fig.height=2.5, fig.width=5.5, echo=FALSE}
n <- 100
Deductible <-seq(0, 1, length=n)
dat <- data.frame(
  Deductible=rep(Deductible, 5),
  Curve=gl(5, n, ordered = TRUE,
           labels=c("Y1", "Y2", "Y3", "Y4", "Lloyd's")),
  G = c(mbbefdCurve(Deductible, 1.5),
        mbbefdCurve(Deductible, 2.0),
        mbbefdCurve(Deductible, 3.0),
        mbbefdCurve(Deductible, 4.0),
        mbbefdCurve(Deductible, 5.0))  
)
...
The following example data is taken from the earlier mentioned Swiss Re
technical paper. 
...
The risk profile given by the cedent is from the year 2002. However, instead of re-indexing the historical data to 2004, we back-index
...
retainedLoss <- mbbefdCurve(retainedDed, 4)
prem <- 1194 * (1 - retainedLoss)
```
 

 
In order to use the exposure curves we need the expected loss and the deductible as % of the MPL. We have to consider three scenarios:
...
### Loss above deductible, but below limit
...
`r round(mbbefdCurve(D/L, 4),1)`%. 

To calculate the premium we first have to derive the proportion of
premium that is attached to the risk below the limit: 4,155k/8,100k $\cdot$ 1,918k=`r round(4155/81000*1918,1)`k.

Multiplying the premium with the reinsurer's share of the loss gives
us a premium of CHF `r round(4155/81000*1918*(1-mbbefdCurve(D/L, 4)),1)`k.

### Rate on Line
For the overall portfolio we can calculate the rate on line, the loss cost
...
XL_ROL <- function(Deductible, Limit, MaxMPL, 
  ...
  LossShare <- 1 - apply(cbind(DedPerMPL, C), 1, 
                         function(x) mbbefdCurve(x[1], x[2]))
