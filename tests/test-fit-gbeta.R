...
if(FALSE)
{
  ...
  par(mfrow=c(1,3))
  llcurve(min.arg=0.1, max.arg=5, plot.arg="shape0", fix.arg=as.list(f1$estimate[-1]), plot.np=50, 
          data=x, distr="gbeta", enhance=FALSE)
  abline(v=c(2, f1$estimate["shape0"]), col=c("green", "red"))
  llcurve(min.arg=0.1, max.arg=4, plot.arg="shape1", fix.arg=as.list(f1$estimate[-2]), plot.np=50, 
          data=x, distr="gbeta", enhance=FALSE)
  abline(v=c(2, f1$estimate["shape1"]), col=c("green", "red"))
  llcurve(min.arg=0.1, max.arg=4, plot.arg="shape2", fix.arg=as.list(f1$estimate[-3]), plot.np=50, 
          data=x, distr="gbeta", enhance=FALSE)
